...
const recruiterPersonalBase = z.object({
  firstName: z.string().min(1, "First name is required"),
  lastName: z.string().min(1, "Last name is required"),
  email: z
    .string({ required_error: "Email is required" })
...
export const recruiterPersonalSchema = recruiterPersonalBase.refine(
...
const recruiterCompanyBase = z.object({
...
export const recruiterCompanySchema = recruiterCompanyBase;

export const recruiterRegistrationSchema = recruiterPersonalBase
  .merge(recruiterCompanyBase)
  .refine((data) => data.password === data.confirmPassword, {
    message: "Passwords do not match",
    path: ["confirmPassword"],
  });
